...
const AttarCustomerTable = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/Attar_form_fetch.php`);
  ...
  const formatCell = (value, key) => {
    ...
    if (key === "intensity") {
      ...
      return (
        <span
          className={`inline-block px-2.5 py-1 rounded text-xs font-medium ${colorClass}`}
        >
          {value}
        </span>
      );
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = "attar_customers.csv";
    a.click();
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, "attar_customers.xlsx");
    setShowExportMenu(false);
  };
  ...
  const exportPDF = () => {
    try {
      const doc = new jsPDF();
      doc.text("Attar Customer Data", 14, 15);
      autoTable(doc, {
      ...
      <div className="mx-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
          <h1 className="text-3xl font-semibold mb-1 text-gray-900">
            Attar Customer Preferences
          </h1>
          <p className="text-gray-500 text-sm">
            Manage and export attar customer data
          </p>
        </div>
...
export default AttarCustomerTable;
