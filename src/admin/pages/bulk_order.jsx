...
const BulkOrder = () => {
    ...
    if (searchTerm) {
      ...
      const filtered = orders.filter((order) =>
        ALL_COLUMNS.some((col) =>
          (order[col.key] || "").toString().toLowerCase().includes(term)
        )
      );
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/get_bulk_orders.php`);
  ...
  const getVisibleColumns = () => {
    ...
    else if (windowWidth > 1200) visibleCount = 6;
    else if (windowWidth > 992) visibleCount = 5;
    else if (windowWidth > 768) visibleCount = 4;
    else if (windowWidth > 600) visibleCount = 3;
    else visibleCount = 2;
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, "bulk_orders.xlsx");
    setShowExportMenu(false);
  };
  ...
  const exportPDF = () => {
    try {
      const doc = new jsPDF();
      doc.text("Bulk Order Data", 14, 15);
      autoTable(doc, {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 p-5 flex flex-col items-center justify-center">
        ...
        <p className="text-gray-600 mt-3">Loading bulk orders...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 p-5 flex flex-col items-center justify-center">
        <p className="text-gray-500 text-lg">⚠️ Error fetching bulk orders</p>
      </div>
    );
  }
      ...
      <div className="mx-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
          ...
          <p className="text-gray-500 text-sm">
            Manage and export customer bulk order enquiries
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          {filteredData.length === 0 ? (
            <div className="text-center py-10 text-gray-400">
              <h3 className="text-lg mb-1.5">No orders found</h3>
                  ...
                  <React.Fragment key={index}>
                    <tr className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      {visible.map((col) => (
                        <td
                          key={col.key}
                          className={`px-4 py-3.5 text-gray-800 text-sm ${
                            col.key === "full_name"
