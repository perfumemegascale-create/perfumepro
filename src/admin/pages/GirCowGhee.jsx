...
const GirCowGhee = () => {
  const ALL_COLUMNS = [
    ...
    { key: "currentGhee", label: "Current Ghee" },
    { key: "monthlyUsage", label: "Monthly Usage" },
    { key: "familyMembers", label: "Family Members" },
    { key: "requiredGhee", label: "Required Ghee" },
    { key: "interestedType", label: "Interested Type" },
    ...
    if (searchTerm) {
      ...
      const filtered = submissions.filter((item) =>
        ALL_COLUMNS.some((col) =>
          (item[col.key] || "").toString().toLowerCase().includes(term)
        )
      );
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/get_ghee_submissions.php`);
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = "gir_cow_ghee_submissions.csv";
    a.click();
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, "gir_cow_ghee_submissions.xlsx");
    setShowExportMenu(false);
  };
  ...
  const exportPDF = () => {
    try {
      const doc = new jsPDF();
      doc.text("Gir Cow Ghee Submissions", 14, 15);
      autoTable(doc, {
      ...
      doc.save("gir_cow_ghee_submissions.pdf");
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-5 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 border-3 border-gray-200 border-t-[#5B3A29] rounded-full animate-spin"></div>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 p-5 flex items-center justify-center">
        <div className="bg-white rounded-lg shadow-sm text-center py-10 px-5">
          <h3 className="text-lg mb-1.5 text-gray-700">⚠️ Error Loading Data</h3>
          <p className="text-gray-500">Unable to fetch data. Please try again later.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mx-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
          <h1 className="text-3xl font-semibold mb-1 text-gray-900">
            Gir Cow Ghee Form Submissions
          </h1>
          <p className="text-gray-500 text-sm">
            Manage and export customer ghee preferences
          </p>
        </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="relative">
              <button
                ...
                className="bg-[#5B3A29] text-white px-5 py-2.5 rounded-md font-medium flex items-center gap-1.5 hover:bg-[#4A2E20] transition-colors text-sm"
              >
                <span>⬇️</span>
                Export Data
              </button>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          {filteredData.length === 0 ? (
            <div className="text-center py-10 px-5 text-gray-400">
              <h3 className="text-lg mb-1.5">No records found</h3>
                  ...
                  <React.Fragment key={index}>
                    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      {visible.map((col) => (
                        <td
                          key={col.key}
                          className={`px-4 py-3.5 text-gray-800 text-sm ${col.key === "name"
